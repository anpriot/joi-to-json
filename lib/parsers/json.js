...
class JoiJsonSchemaParser {
  constructor(opts = {}) {
    ...
    this.includeMetaPropKeys = Boolean(opts.includeMetaPropKeys)
    if (typeof opts.includeMetaPropKeys === 'string' && opts.includeMetaPropKeys !== '' ) {
      this.includeMetaPropKeysFromParent = opts.includeMetaPropKeys
    }
  ...
  _setMetaProperties(schema, joiSpec) {
    _.forEach(joiSpec.metas, (m) => {
      ...
      if (this.includeMetaPropKeys) {
        let parent

        if (
          this.includeMetaPropKeysFromParent &&
          m[this.includeMetaPropKeysFromParent] &&
          typeof m[this.includeMetaPropKeysFromParent] === 'object'
        ) {
            parent = m[this.includeMetaPropKeysFromParent]
        } 
        else if (!this.includeMetaPropKeysFromParent) {
            parent = m
        }

        if (parent) {
          Object.entries(parent).forEach(([k, v]) => schema[k] = v)
        }
      }
    })
  }
